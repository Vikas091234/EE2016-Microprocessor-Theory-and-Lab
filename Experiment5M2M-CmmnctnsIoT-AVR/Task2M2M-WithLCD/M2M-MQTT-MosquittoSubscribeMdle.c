...
void lcd_send(unsigned char value, unsigned char mode) {
    ...
    if (mode == 0) {
        LCD_PORT_A.OUTCLR = ....; // command
    } else {
        LCD_PORT_A.OUTSET = ....; // data
    }
...
void lcd_init(void) {
    ...
    lcd_send(0x..., 0); // Initialize LCD in 4-bit mode
    lcd_send(0x28, 0); // 2 lines, 5x8 matrix
    lcd_send(0x.., 0); // Display on, cursor off
    lcd_send(0x06, 0); // Increment cursor
    lcd_send(0x.., 0); // Clear display
...
void lcd_clear(void) {
    lcd_send(0x.., 0); // Clear display command
    _delay_ms(2);      // Wait for the LCD to process the clear command
}
void lcd_set_cursor(uint8_t col, uint8_t row) {
    if (row == 0) {
        lcd_send((col & 0x0F) | 0x.., 0);
    } else {
        lcd_send((col & 0x0F) | 0x.., 0);
    }
}
